...
└── Workflows/
    ...
    ├── 02-analyze.md           Phase 2 分析阶段（6 步）：启动并行 agents → 提取模式 → 构建知识地图 → 可迁移模式识别 → 规划章节
    └── 03-compose.md           Phase 3 撰写阶段（6 步）：确认章节 → 逐章写作 → 可选终章"工程迁移" → 去 AI 味审查 → 输出文件
```
...
从 ljg-skill-xray-article 的"迁移"概念适配而来。在分析阶段（02-analyze 5.6）识别可跨项目复用的设计模式，在撰写阶段（03-compose 2.4）作为可选终章输出。

**与 xray-article 的区别**：xray-article 的迁移面向认知升级（个人化、跨域思维模型），source-dive 的迁移面向工程实践（设计模式、架构公式、避坑清单）。不引入个人化元素，不搞升华，每个模式必须有源码证据。

**可选机制**：迁移素材不足时终章降级为简短 takeaway 合并到总结章节，不硬编空话。
...
| 去AI味需要逐章检查 + 全文复查 | 03-compose Step 2.5 + Step 3 | 两遍检查比一遍更干净 |
| 每章写完立即去 AI 味比攒到最后改效率高 | 03-compose Step 2.5 | 减少全文级返工 |
